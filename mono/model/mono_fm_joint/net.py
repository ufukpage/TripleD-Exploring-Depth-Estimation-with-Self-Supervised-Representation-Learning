...
from .diffnet_encoder import _hrnet
from .depth_decoder import DepthDecoder, HRDepthDecoder, DIFFDepthDecoder
...
class mono_fm_joint(nn.Module):
    def __init__(self, options):
        ...
        if not self.opt.get('use_diffnet', False):
            self.DepthEncoder = DepthEncoder(self.opt.depth_num_layers,
                                             self.opt.depth_pretrained_path)
        else:
            self.DepthEncoder = _hrnet('hrnet'+str(self.opt.depth_num_layers), True, False,
                                       pretrained_path=self.opt.depth_pretrained_path)
            self.DepthEncode.num_ch_enc = [64, 18, 36, 72, 144]
        ...
        if self.opt.get('use_diffnet', False):
            self.DepthDecoder = DIFFDepthDecoder(self.DepthEncoder.num_ch_enc)
        ...
        self.Encoder = Encoder(self.opt.get('extractor_num_layers', 50), self.opt.extractor_pretrained_path)
    ...
    def compute_losses(self, inputs, outputs, features):
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.automask:
                # uses source input images as prediction
                # if its too small that means, pixel is stationary, thus can replace prediction to prevent gradient flow
                # replacing those pixels at the next stage implicitly masks them out.
            ...
            """
            minimum (auto-encoder)perceptional loss
            """
            for frame_id in self.opt.frame_ids[1:]:
    ...
    def predict_poses(self, inputs, pose_feats=None):
        outputs = {}
        #[192,640] for kitti
        if pose_feats is None:
            pose_feats = {f_i: F.interpolate(inputs["color_aug", f_i, 0], [192, 640],
                                             mode="bilinear", align_corners=False) for f_i in self.opt.frame_ids}
