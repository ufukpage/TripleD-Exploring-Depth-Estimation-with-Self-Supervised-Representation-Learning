...
class autoencoder(nn.Module):
    def __init__(self, options):
        ...
        self.ssim = SSIM()
        os.makedirs('./autoencoder_test', exist_ok=True)
        self.count = 0
        self.save_fig = False
    ...
    def compute_losses(self, inputs, outputs, features):
        ...
        for scale in self.opt.scales:
            ...
            if self.count % interval == 0 and self.save_fig:
                img_path = os.path.join('./autoencoder_test', 'auto_{:0>4d}_{}.png'.format(self.count // interval, scale))
                plt.imsave(img_path, pred[0].transpose(0,1).transpose(1,2).data.cpu().numpy())
                img_path = os.path.join('./autoencoder_test', 'img_{:0>4d}_{}.png'.format(self.count // interval, scale))
