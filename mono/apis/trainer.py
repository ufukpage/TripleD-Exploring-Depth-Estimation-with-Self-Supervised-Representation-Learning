...
from mono.core import (DistOptimizerHook, DistEvalMonoHook, NonDistEvalHook, NonDistSegmentationEvalHook,
                       DistEvalSegmentationHook)
...
def batch_processor(model, data, train_mode):
    ...
    outputs = dict(loss=loss,
                   ...
                   num_samples=len(data[('color', 0 , 0)].data) if isinstance(data, dict) else len(data[0]))
...
def _dist_train(model, dataset_train, dataset_val, cfg, validate=False):
    ...
    model = MMDistributedDataParallel(model.cuda(), find_unused_parameters=find_unused_parameters,
    ...
    if validate:
        ...
        if 'num_classes' in cfg:
            runner.register_hook(DistEvalSegmentationHook(dataset_val, interval, cfg))
        else:
...
def _non_dist_train(model, dataset_train, dataset_val, cfg, validate=False):
    ...
    # put model on gpus
    model = MMDataParallel(model, device_ids=list(range(cfg.gpus.__len__()))).cuda()
    # build runner
    ...
    if validate:
        print('validate........................')
        if 'num_classes' in cfg:
            runner.register_hook(NonDistSegmentationEvalHook(dataset_val, cfg))
        else:
