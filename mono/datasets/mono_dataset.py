...
import random
import numpy as np
from PIL import Image, ImageCms  # using pillow-simd for increased speed
...
def rgb2lab(im):
    ...
    return ImageCms.applyTransform(im, transform)
...
class MonoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in self.frame_idxs:
            ...
            if self.cfg.get('add_lab', False):
                inputs[("color_lab", i, -1)] = rgb2lab(inputs[("color", i, -1)])
...
class MonoDatasetImRot(MonoDataset):
    """Superclass for monocular dataloaders
    ...
    def __init__(self,
        ...
        super(MonoDatasetImRot, self).__init__(data_path, filenames, height, width, frame_idxs, is_train=False,
                                               img_ext='.jpg', gt_depth_path=None )
    ...
    def __getitem__(self, index):
        ...
        rotated_imgs = [
            ...
            rotate_img(inputs["color_aug", 0, 0], 90),
            rotate_img(inputs["color_aug", 0, 0], 180),
            rotate_img(inputs["color_aug", 0, 0], 270)
        ]
        rotation_labels = torch.LongTensor([0, 1, 2, 3])

        inputs['rotate_img_stack'] = rotated_imgs
        inputs['rotation_labels'] = rotation_labels
