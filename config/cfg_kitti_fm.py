...
FRAME_IDS = [0, -1, 1] #FRAME_IDS = [0, -1, 1, 's']#0 refers to current frame, -1 and 1 refer to temperally adjacent frames, 's' refers to stereo adjacent frame.
...
model = dict(
    ...
    depth_pretrained_path = None, #'F:\\pretrained\\simclrresnet50.pth', #'F:\\pretrained\\resnet{}.pth'.format(DEPTH_LAYERS),# pretrained weights for resnet
    pose_pretrained_path =  None, #'F:\\pretrained\\resnet{}.pth'.format(POSE_LAYERS),# pretrained weights for resnet
    extractor_pretrained_path =  None, #'F:\\pretrained\\simclrresnet50.pth',#'F:\\pretrained\\autoencoder.pth',# pretrained weights for autoencoder
