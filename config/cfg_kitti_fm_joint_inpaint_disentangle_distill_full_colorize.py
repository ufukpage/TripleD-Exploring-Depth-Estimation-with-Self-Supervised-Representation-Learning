...
model = dict(
    name = 'mono_fm_joint_inpaint_disentangle_distill_sep_colorize',
    ...
    disentangle_layers=[False, False, False, False, False],
    skip_connection_multiplier=1,
    colorize_pretrained_path='F:\\pretrained\\resnet{}.pth'.format(DEPTH_LAYERS),  # pretrained weights for resnet
    colorize_weight=5e-3,
    use_distill_mask=True,
    img_reconstruct_weight=1,
    colorize_num_layers=COLORIZE_LAYERS,
)
